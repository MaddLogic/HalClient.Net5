...
namespace HalClient.Net5.Tests
{
		...
		private const string JsonNullLinks = @"
{
	...
	""_embedded"": {
		...
		public void EmbeddedParsing_ParsesTheCorrectNumberOfRels()
		{
			...
			Assert.Single(result.EmbeddedResources.Select(x => x.Rel).Distinct());
		}
		...
		public void LinkParsing_ParsesCorrectRels()
		{
			...
			Assert.Contains(result.Links, x => x.Rel == "self");
			Assert.Contains(result.Links, x => x.Rel == "curies");
			Assert.Contains(result.Links, x => x.Rel == "next");
			Assert.Contains(result.Links, x => x.Rel == "ea:find");
			Assert.Contains(result.Links, x => x.Rel == "ea:admin");
		}
		...
		public void LinkParsing_ParsesCorrectAdminTitles()
		{
			...
			Assert.Contains(result.Links.Where(x => x.Rel == "ea:admin"), x => x.Title.Equals("Kate"));
			Assert.Contains(result.Links.Where(x => x.Rel == "ea:admin"), x => x.Title.Equals("Fred"));
		}
